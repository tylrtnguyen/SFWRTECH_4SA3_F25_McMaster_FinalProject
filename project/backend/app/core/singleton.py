            ...
            with self._lock:
                if self.client is None:
                    if not settings.SUPABASE_DATABASE_URL or not settings.SUPABASE_DATABASE_API_KEY:
                        raise ValueError("Supabase credentials not configured. Set SUPABASE_DATABASE_URL and SUPABASE_DATABASE_API_KEY in .env")
...
class APIConnectionManager:
    """
    Singleton for managing API connections (Ruvia Trust, LinkedIn, Indeed)
